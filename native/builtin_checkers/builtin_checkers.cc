...
public:
    BuiltinCheckerWorker(Napi::Function &&callback, std::string &&outputFile, std::string &&answerFile,
            std::function<void()> checkerFunction)
    ...
    void Execute() {
        ...
        try {
            ...
            const size_t BUFFER_SIZE = static_cast<const size_t>(64 * 1024);
...
void runBuiltinChecker(const Napi::CallbackInfo &info, std::function<void()> checkerFunction) {
    auto *worker = new BuiltinCheckerWorker(info[3].As<Napi::Function>(), info[0].As<Napi::String>().Utf8Value(),
            info[1].As<Napi::String>().Utf8Value(), checkerFunction);
    worker->Queue();
}
